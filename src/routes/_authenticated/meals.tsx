...
function Meals() {
  ...
  const addFood = (f: string) => {
    ...
    if (foods.length >= 30) return toast.error("Max 30 foods");
    setFoods((s) => [...s, v]); setInput("");
  };

  const generate = async () => {
    if (foods.length === 0) return toast.error("Add at least one food");
    setBusy(true);
    try {
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      <header>
        <h1 className="text-2xl font-black sm:text-3xl">AI Meal Planner</h1>
        <p className="text-sm text-muted-foreground">List foods you have - we'll build a day's plan that hits your targets.</p>
      </header>

      <section className="rounded-2xl border border-border/60 bg-card/70 p-5">
        <h3 className="font-bold">What's available?</h3>
        <form className="mt-3 flex gap-2" onSubmit={(e) => { e.preventDefault(); addFood(input); }}>
          <Input value={input} onChange={(e) => setInput(e.target.value)} placeholder="Add a food (e.g. Quinoa)" maxLength={40} />
        ...
        <div className="mt-4">
          <div className="text-[10px] uppercase tracking-wider text-muted-foreground">Quick add</div>
        ...
        <Button variant="hero" size="lg" className="mt-5 w-full sm:w-auto" disabled={busy} onClick={generate}>
          ...
          Generate plan ({profile.targets.calories} kcal / {profile.targets.proteinG}g protein)
        </Button>
      </section>
        ...
        {plan && (
          <motion.section initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
            ...
            <div className="rounded-2xl border border-primary/30 bg-primary/5 p-5">
              <div className="flex items-center gap-2 text-sm font-bold text-primary"><Sparkles className="h-4 w-4" /> Your AI Plan</div>
            ...
            {plan.tips?.length > 0 && (
              <div className="rounded-2xl border border-border/60 bg-card/70 p-5">
                <div className="flex items-center gap-2 font-bold"><Lightbulb className="h-4 w-4 text-amber-400" /> Coach tips</div>
